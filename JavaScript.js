...
function lanzar(e,delay=0){if(!e)return;setTimeout(()=>{if(!estado.jugando||estado.pausado||estado.terminado)return;e.dataset.tocado="0";e.style.top=Math.max(7,Math.random()*78)+"%";e.style.left="-12%";e.style.transition="left 7.5s linear";requestAnimationFrame(()=>e.style.left="78%");},delay);}
function iniciarMeteoritos(){meteoritos.forEach((id,i)=>{const e=$(id);if(!e)return;ocultar(e);lanzar(e,i*1800);});}
...
function configurar(){barra("Puntaje","BarraProgresoLvl1","ProgresoInternoLvl1");$("Play")?.addEventListener("click",()=>cuenta("Contenedor_contador",iniciarNivel1));$("Pause")?.addEventListener("click",pausarNivel1);meteoritos.forEach(conectar);$("NEXT")?.addEventListener("click",()=>{if($("NIVEL_01").style.display!=="none"&&estado.terminado){$("NIVEL_01").style.display="none";$("NIVEL_02").style.display="flex";$("NIVEL3").style.display="none";$("NEXT").style.display="none";window.prepararNivel2?.();}else if($("NIVEL_02").style.display!=="none"&&window.nivel2Terminado?.()){$("NIVEL_02").style.display="none";$("NIVEL3").style.display="flex";$("NEXT").style.display="none";window.prepararNivel3?.();}});$("NEXT").style.display="none";actualizarHUD();}
