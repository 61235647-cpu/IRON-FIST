...
function lanzar(e,delay=0){if(!e)return;setTimeout(()=>{if(!s.jugando||s.pausado||s.terminado)return;e.dataset.tocado="0";e.style.top=Math.max(6,Math.random()*80)+"%";e.style.transition="none";e.dataset.progreso="0";animarMeteorito(e,0);},delay);}
function animarMeteorito(e,progresoInicial=0){
  if(!e)return;
  cancelAnimationFrame(e._ironFistFrame);
  const duracion=3.3*1000;
